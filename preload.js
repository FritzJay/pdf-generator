const { generatePDFs, getAvailablePDFTypes } = require("./lib/pdf");
...
function _submitForm() {
  ...
  const { SourceDirectory, DestinationDirectory } = _getSettingsFromPage();
...
function _initializeProfiles() {
  ...
  document.getElementById(
    "profile-save-button"
  ).onclick = _handleProfilesSaveButtonOnClick;
  document.getElementById(
    "profile-delete-button"
  ).onclick = _handleProfilesDeleteButtonOnClick;
}
...
function _saveProfile(name) {
  if (
    confirm(`Are you sure you want to create/update the profile "${name}"?`)
  ) {
    const info = _getInfoFromPage();
...
function _isValidProfileName(name) {
  ...
  if (
    nameWithoutSpaces !== NO_PROFILE_SELECTED_VALUE &&
    nameWithoutSpaces !== undefined &&
...
function _initializeSettings() {
  ...
  ).onchange = _handleSourceDirectoryChange;
  document.getElementById(
    "destination-directory-button"
  ).onchange = _handleDestinationDirectoryChange;
  document.getElementById(
    "source-directory-input"
  ).value = DEFAULT_SOURCE_DIRECTORY;
  document.getElementById(
...
function _initializePDFs() {
  const sourceDirectory = document.getElementById("source-directory-input")
    .value;
...
function _addPDFTypesToFieldset(pdfTypes, formsFieldset) {
  if (pdfTypes.length < 1) {
    _addMessageToElement(
      "The source directory does not contain any pdfs.",
      formsFieldset,
      {
        id: "pdf-types-message"
      }
